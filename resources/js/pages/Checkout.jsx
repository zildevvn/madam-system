...
import { updateQuantity, checkoutOrderAsync, cancelOrderAsync, updateItemNote, removeFromCart, selectSelectedItems, updateOrderTableAsync, clearCart, createOrderAsync, selectOriginalItems } from '../store/slices/orderSlice';
    ...
    const handleCheckout = async () => {
        try {
            ...
            if (isModified || !wasConfirmed) {
                // Identify all current drinks
                const allDrinks = selectedItems.filter(item => item.type === 'drink');

                // --- BEVERAGE CHANGE DETECTION ---
                // We compare the current cart against the baseline (originalItems)

                // Check if any existing/new drinks have been added or modified
                const hasModifiedDrinks = allDrinks.some(item => {
                    const original = originalItems[item.id];
                    if (!original) return true; // Brand new drink added to existing order

                    const isQtyDiff = Number(original.quantity) !== Number(item.quantity);
                    const isNoteDiff = (original.note || '').trim() !== (item.note || '').trim();

                    return isQtyDiff || isNoteDiff;
                });

                // Check if any previously ordered drinks were removed
                const hasRemovedDrinks = Object.keys(originalItems).some(id => {
                    const original = originalItems[id];
                    // If it was a drink but is no longer in the cart, it's a removal
