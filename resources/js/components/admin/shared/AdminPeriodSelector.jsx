...
const AdminPeriodSelector = ({ periods, currentPeriod, onPeriodChange }) => {
    return (
        <div className="flex bg-[#f1f5f9] p-1 rounded-full w-full lg:w-auto">
            {periods.map((p) => (
                <button
                    key={p.id}
                    onClick={() => onPeriodChange(p.id)}
                    className={`flex-1 lg:flex-none px-6 py-2 rounded-full text-[11px] font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap ${currentPeriod === p.id
