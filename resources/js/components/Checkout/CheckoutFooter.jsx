            ...
            <div className="max-w-2xl mx-auto">
                <div className="flex justify-between items-end mb-4">
                    <span className="font-extrabold label-md text-on-surface-variant uppercase tracking-wider">
                        {isSplitMode ? `Đã chọn: ${splitItemsCount}` : `Số lượng: ${totalQuantity}`}
                    </span>
                    ...
                    {isSplitMode ? (
                        <>
                            ...
                            <button
                                ...
                                disabled={splitItemsCount === 0}
                                className={`btn-save mdt-btn w-full text-xs ${splitItemsCount === 0 ? 'opacity-50 grayscale' : ''}`}
                            >
                                Xác nhận tách ({splitItemsCount})
                            </button>
                        </>
                    ) : (
                        <>
                            {isConfirmed && !isModified && (
                                <button
                                    ...
                                    className="mdt-btn-outline w-full text-xs flex items-center justify-center gap-1.5"
                                >
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M8 3v18M16 3v18M3 8h18M3 16h18"/></svg>
                                    Tách đơn
                                </button>
                            )}
                            <button
                            ...
                            <button
                                ...
                                className={`btn-save mdt-btn w-full text-xs ${(!hasItems || (isConfirmed && !isModified && !isTableChanged && !isMergeChanged)) ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
                            >
